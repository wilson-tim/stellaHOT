create or replace PROCEDURE        sp_l_bsp_transaction_203 (fileToProcess VARCHAR2 ) AS 
...
--
--
-- @I:\MI\DarrenScott\Workspace\UK\Database\dtwlive\stella\procs\SP_L_BSP_TRANSACTION_v203_live.prc
--
  ...
  --
     ...
     SELECT isRecordTypeSeqValid(v_prevrecid,v_recid) 
        ...
        -- dbms_output.put_line( SUBSTR(v_data_text,7,6));
                v_sequence := TO_NUMBER( SUBSTR(v_data_text,7,5));
         ...
         END IF;
                ...
                ELSIF v_recid = 'BKT' THEN
                    ...
                    v_sequence := TO_NUMBER(SUBSTR(v_data_text,8,5));
...
-- Reset database variables
                    ...
                    IF core_dataw.is_number(SUBSTR(v_data_text,38,3)) = 0 THEN 
                        dbms_output.put_line('ERROR datafile: ' || fileToProcess || ' BKT airline non-numeric: ' || SUBSTR(v_data_text,38,3) ||
                                             ' (record was: ' || v_data_text || ')');
                        RAISE invalid_row;
                    ELSE
                       v_airline_num := SUBSTR(v_data_text,38,3);
                    END IF;                    

                    v_bsp_crs_code := SUBSTR(v_data_text,88,4);                 

                    --source of transaction
                    v_netremitind := SUBSTR(v_data_text,31,2);   
...
--                        countBKS24 ++; 
                        ...
                        v_iata_num         := TO_NUMBER(SUBSTR(v_data_text,52,9));  
                       ...
                       IF SUBSTR(v_data_text,49,3) = 'CNJ' THEN
                       ...
                       -- dbms_output.put_line('Conjunction ind = N');
                        ...
                        v_transaction_code := SUBSTR(v_data_text,80,3); 
        ...
        --                            return 1;
                                ...
                                v_commissionable_amt :=  v_commissionable_amt + BSP_amt(SUBSTR(v_data_text,29,11));  

                                -- this is the selling fare

                                v_net_fare_amt := v_net_fare_amt+BSP_amt(SUBSTR(v_data_text,40,11));
                                ...
                                v_tax_type := SUBSTR(v_data_text,52,2);
                                --dbms_output.put_line('Tax Type1: '||v_tax_type);  

                                --dbms_output.put_line(' 6 ');
                                IF v_tax_type != 'CP' THEN 

                                    v_tax1 := BSP_amt(SUBSTR(v_data_text,60,11));                                    
                                ...
                                -- CP airline penalty -- only usually used for RFND transactions for Galileo sourced refunds
                                    ...
                                    v_airline_penalty_amt :=  v_airline_penalty_amt + BSP_amt(SUBSTR(v_data_text,60,11));
                                END IF;

                                v_tax_type := SUBSTR(v_data_text,71,2);
                                --dbms_output.put_line('Tax Type2: '||v_tax_type);  

                                IF v_tax_type !='CP' THEN
                                   v_tax2 := BSP_amt(SUBSTR(v_data_text,79,11));  
                                ...
                                --dbms_output.put_line('Airline_penalty 2 99,11: '||SUBSTR(v_data_text,99,11));
                                    v_airline_penalty_amt2 := v_airline_penalty_amt2 + BSP_amt(SUBSTR(v_data_text,79,11)); ----------CHECK HERE FOR CP VALUE-------------------
                                END IF;
                                ...
                                v_balance_payable_amt := v_balance_payable_amt + BSP_amt(SUBSTR(v_data_text,90,11));
        ...
        --                        v_commission_amt := v_commission_amt + BSP_amt(SUBSTR(v_data_text,29,11);
                              ...
                              --totalLateReportingAmt := totalLateReportingAmt + BSP_amt(SUBSTR(v_data_text,101,11));
                                ...
                                v_tax_amt              := v_tax_amt + v_tax1 + v_tax2;
                                --dbms_output.put_line('Total Tax - ' ||TO_CHAR(v_tax_amt));                        
                                v_tax1                 := 0;
                                v_tax2                 := 0;
                                v_airline_penalty_amt  := v_airline_penalty_amt + v_airline_penalty_amt2;  
                                v_airline_penalty_amt2 := 0;
                                v_ccy_code := SUBSTR(v_data_text,117,3);
...
--                             return 1;
                        END IF;
                        dbms_output.put_line('RecType - ' || TO_CHAR(c1_rec.rec_type));                        
                        v_commission_amt := v_commission_amt + BSP_amt(SUBSTR(v_data_text,43,11));
...
--                        v_commissionable_amt   := v_balance_payable_amt - v_tax_amt;
                        ...
                        IF SUBSTR(v_data_text,117,3) != v_ccy_code THEN
...
--                       v_recdate := TO_NUMBER(SUBSTR(v_data_text,1,6));
                        ...
                        --dbms_output.put_line(' Conj 1 ' || v_conjunction_ind ) ;    
                            IF v_conjunction_ind = 'Y' THEN
                               ...
                               v_remittance_amt := BSP_amt(SUBSTR(v_data_text,99,11));
